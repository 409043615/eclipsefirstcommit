...
public class Calculator extends JFrame implements ActionListener {
	...
	private void init() {
		...
		// 下面进行计算器的整体布局，将command画板放在计算器的中部，
		// 将文本框放在北部，将calms画板放在计算器的西部。

		// 新建一个大的画板，将上面建立的command画板放在该画板内
	...
	private void handleBackspace() {
		...
		if (i > 0) {
			...
			if (text.length() == 0) {
				...
				// 显示新的文本
				resultText.setText(text);//使退格键运行后显示界面//
			}
		}
	}
